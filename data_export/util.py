...
def compute_hours_between_clock_times(clocked_in, clocked_out):
    if isinstance(clocked_in, int) and isinstance(clocked_out, int):
        clocked_in = datetime.fromtimestamp(
            clocked_in / 1000.0).strftime('%H:%M:%S')
        clocked_out = datetime.fromtimestamp(
            clocked_out / 1000.0).strftime('%H:%M:%S')
    ...
    if clocked_in > clocked_out:
        return round((hours_worked.total_seconds()+86400) / 3600.0, 2)
    # print(hours_worked)
